...
export const isAlreadyFollowingMessage = (message) => {
 ...
 */
export const isNotFoundMessage = (message) => {
    const lower = message.toLowerCase();
 ...
 * nothing.
 */
export const resolveOutgoingCancel = async (deps) => {
    const first = await deps.reject();
    if (first.success)
        return { ok: true, via: "reject", message: "Follow request cancelled" };
    ...
    if (second.success)
        return { ok: true, via: "unfollow", message: "Follow request cancelled" };
 ...
 * remove.
 */
export const resolveUnfollow = async (deps) => {
    const first = await deps.unfollow();
    if (first.success)
        return { ok: true, via: "unfollow", message: "Unfollowed" };
    if (!isNotFoundMessage(msg(first))) {
