...
import { GetApiCommentsPostPostIdQueryParams, PostApiCommentsBody, PostApiCommentsReplyBody, PutApiCommentsCommentIdBody, } from "./comments.js";
...
describe("PostApiCommentsBody (create)", () => {
    it("accepts the canonical { postId, content } pair", () => {
        assert.deepEqual(PostApiCommentsBody.parse({ postId: "p-1", content: "Nice!" }), { postId: "p-1", content: "Nice!" });
    });
    it("strips unknown keys", () => {
        assert.deepEqual(PostApiCommentsBody.parse({ postId: "p-1", content: "x", authorId: "u-1" }), { postId: "p-1", content: "x" });
    });
});
describe("PostApiCommentsReplyBody", () => {
    it("accepts the canonical { postId, content, parentId } triple", () => {
        assert.deepEqual(PostApiCommentsReplyBody.parse({ postId: "p-1", content: "Agreed", parentId: "c-1" }), { postId: "p-1", content: "Agreed", parentId: "c-1" });
    });
});
describe("PutApiCommentsCommentIdBody (update)", () => {
    it("accepts the canonical { id, content } pair", () => {
        assert.deepEqual(PutApiCommentsCommentIdBody.parse({ id: "c-1", content: "Edited" }), { id: "c-1", content: "Edited" });
    });
});
