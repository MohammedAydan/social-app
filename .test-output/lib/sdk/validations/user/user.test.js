...
import { GetApiUserSearchQueryParams, PostApiUserChangePasswordBody, PostApiUserForgetPasswordBody, PostApiUserRegisterBody, PostApiUserResetPasswordBody, PostApiUserSignInBody, PutApiUserUpdateUserBody, } from "./user.js";
/**
...
describe("PostApiUserRegisterBody", () => {
    it("accepts the canonical shape with ISO birthDate", () => {
        assert.deepEqual(PostApiUserRegisterBody.parse({
            firstName: "Ahmed",
            lastName: "Ali",
            userName: "ahmed",
            userGender: "male",
...
describe("PostApiUserSignInBody", () => {
    it("accepts the canonical { email, password } pair", () => {
        assert.deepEqual(PostApiUserSignInBody.parse({ email: "a@x.com", password: "Secret1!" }), { email: "a@x.com", password: "Secret1!" });
    });
});
describe("PutApiUserUpdateUserBody", () => {
    it("accepts partial updates and strips unknown keys (userGender)", () => {
        assert.deepEqual(PutApiUserUpdateUserBody.parse({ firstName: "Sara", userGender: "female" }), { firstName: "Sara" });
    });
});
describe("Password bodies", () => {
    it("accepts change/forget shapes", () => {
        assert.deepEqual(PostApiUserChangePasswordBody.parse({
            currentPassword: "a",
            newPassword: "b",
            confirmPassword: "b",
        }).currentPassword, "a");
    ...
    it("enforces the reset-password strength policy", () => {
        ...
        assert.deepEqual(PostApiUserResetPasswordBody.parse({ ...base, password: "Secret1!" }).password, "Secret1!");
