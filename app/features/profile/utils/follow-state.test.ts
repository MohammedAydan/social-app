...
const ok = (message = "OK"): PairResult => ({ success: true, message });
const notFound = (message = "An error occurred: Follow relationship not found."): PairResult => ({
    success: false,
    message,
});
const hardFail = (message = "Something broke"): PairResult => ({ success: false, message });
...
describe("isNotFoundMessage", () => {
    it("matches the unfollow 500 envelope", () => {
        assert.equal(
            isNotFoundMessage("An error occurred: Follow relationship not found."),
            true
        );
    });
...
describe("resolveOutgoingCancel (Requested path)", () => {
    const calls: string[] = [];
    const track = (name: string, fn: () => PairResult | Promise<PairResult>) => async () => {
        calls.push(name);
        return fn();
    };
