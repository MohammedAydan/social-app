...
import {
    ...
    PostApiPostsPostIdReportBody,
    PostApiPostsPostIdReportParams,
} from "../../lib/sdk/validations/posts/posts.js";
...
describe("reportPost pre-network param validation (PostApiPostsPostIdReportParams)", () => {
    ...
    it("rejects undefined/null/non-string postIds with a ZodError", () => {
        for (const postId of [undefined, null, 123]) {
            assert.throws(
                () => PostApiPostsPostIdReportParams.parse({ postId: postId as unknown as string }),
                ZodError,
                `expected ZodError for postId=${String(postId)}`
            );
        }
    });
});
...
describe("report payload shape (PostApiPostsPostIdReportBody)", () => {
    ...
    it("accepts a reason with details text (the Other path)", () => {
        assert.deepEqual(
            PostApiPostsPostIdReportBody.parse({ reason: "Other", details: "copied artwork" }),
            { reason: "Other", details: "copied artwork" }
        );
    });
...
describe("cancelReport pre-network param validation (DeleteApiPostsReportsReportIdParams)", () => {
    ...
    it("rejects missing/undefined/null/non-string reportIds with a ZodError", () => {
        ...
        for (const reportId of [undefined, null, 42]) {
            assert.throws(
                () =>
                    DeleteApiPostsReportsReportIdParams.parse({
                        reportId: reportId as unknown as string,
                    }),
                ZodError,
                `expected ZodError for reportId=${String(reportId)}`
            );
        }
    });
});
