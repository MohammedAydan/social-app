...
export const getDisplayName = (user?: NameLike | null): string => {
...
export const getHandle = (user?: NameLike | null): string | null => {
    const handle = user ? clean(user.userName) : "";
    return handle ? `@${handle}` : null;
};

/** Up to 2 uppercase initials from the display name ("Mary Jane" → "MJ"). */
export const getInitials = (user?: NameLike | string | null): string => {
...
export const isDeletedPlaceholder = (content: unknown): boolean =>
    typeof content === "string" && content.trim() === DELETED_CONTENT_PLACEHOLDER;
