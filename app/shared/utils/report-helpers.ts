...
import type { ReportPostRequest } from "../../lib/sdk/models/reportPostRequest.js";
...
const canonicalReason = (reason: string): ReportReason | undefined =>
    REPORT_REASONS.find((r) => r.toLowerCase() === reason.trim().toLowerCase());
 ...
 */
export const parseReportPayload = (input: ReportPostInput): ReportPostRequest =>
    PostApiPostsPostIdReportBody.parse(
        ((): ReportPostRequest => {
            const canonical = canonicalReason(input.reason ?? "");
            if (!canonical) {
                throw new Error(
                    `Invalid report reason: "${input.reason}". Must be one of: ${REPORT_REASONS.join(", ")}.`
                );
            }
...
export type ReportErrorKind = "duplicate" | "self" | "not-found" | "validation" | "auth" | "unknown";
 ...
 */
export const classifyReportError = (message?: string): ReportErrorKind => {
    const lower = (message ?? "").toLowerCase();
    if (lower.includes("duplicate") || lower.includes("already")) return "duplicate";
    if (
        lower.includes("own post") ||
